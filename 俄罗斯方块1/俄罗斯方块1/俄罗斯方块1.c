...
//游戏窗口
#define FrameX 4   //游戏窗口左上角的X轴坐标 （是游戏边框范围）
...
void gotoxy(HANDLE hOut, int x, int y)
{
 ...
 pos.X = x;  //横坐标
 pos.Y = y;  //纵坐标
 SetConsoleCursorPosition(hOut, pos); //设置光标在指定控制台屏幕缓冲区的位置
//控制台屏幕缓冲区句柄   以COORD包含新坐标的位置，新坐标必须在屏幕缓冲区内                                    
}

/******制作游戏窗口******************************************************************/
void make_frame()
{
 HANDLE hOut = GetStdHandle(STD_OUTPUT_HANDLE);  //定义显示器句柄变量，HANDLE是任意对象的句柄

 gotoxy(hOut,FrameX+Frame_width-5,FrameY-2);   //打印游戏名称（拿到控制台屏幕缓冲区的句柄和指定坐标，就可以在任意位置显示要输出的内容）
 printf("俄罗斯方块");
 gotoxy(hOut,FrameX+2*Frame_width+3,FrameY+7);  //打印选择菜单
 printf("**********下一个方块：");
 gotoxy(hOut,FrameX+2*Frame_width+3,FrameY+13); //光标每次移动的距离大概为═的一半故要*2，否则会重叠导致宽度缩短
 printf("**********");
